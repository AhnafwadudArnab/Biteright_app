...
const mealPlanController_1 = require("../Registrations/mealPlanController");
// ── Helper: call handler and capture JSON response ───────────────────────────
function makeReqRes(gender, bmi) {
    const req = {
...
describe("Property 7: BMI meal plan lookup returns consistent structure", () => {
    it("returns bmi_range, category, dailyCalories, non-empty doctorFocus and meals for valid (gender, bmi) pairs", async () => {
        await fc.assert(fc.asyncProperty(fc.constantFrom("male", "female"), 
        // Use BMI values that fall within our seeded ranges
        fc.float({ min: Math.fround(10), max: Math.fround(29.9), noNaN: true }), async (gender, bmi) => {
            const { req, res, next, getBody, getStatus } = makeReqRes(gender, bmi);
            await (0, mealPlanController_1.getMealPlanByBmi)(req, res, next);
            const body = getBody();
            const status = getStatus();
            // Must return 200 with a valid plan (our seed covers 10–29.9)
            if (status !== 200 || !body)
                return true; // skip edge cases
            return (typeof body.bmi_range === "string" &&
                ...
                body.meals.length > 0 &&
                body.meals.every((m) => typeof m.type === "string" &&
